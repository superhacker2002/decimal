...
int main() {
    s21_decimal x = {2147483647,0,0,0};
    s21_decimal y = {6,0,0,0};
    s21_decimal res;
    s21_add(x, y, &res);
    printf("%d %d %d %d", res.bits[0], res.bits[1], res.bits[2], res.bits[3]);
...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    while(n < 31) {
        ...
        int bits = s21_bit(value_1.bits[0], n) + s21_bit(value_2.bits[0],n) + tmp;
        if (bits == 1) {
            s21_shift(&result->bits[i], 1, n);  // сдвигаем нный байт на противоложный, то бишь на 1
            tmp = 0;
        } else if (bits == 2) {
            tmp = 1;
        } else if (bits == 3) {
...
// Сдвигает n-ный байт на shift байтов в числе number
